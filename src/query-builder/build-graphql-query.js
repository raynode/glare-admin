...
export const getArgType = arg => {
  ...
  if (list) return required
    ? gqlTypes.listType(gqlTypes.nonNullType(namedType))
    : gqlTypes.listType(namedType)


  return required
    ? gqlTypes.nonNullType(namedType)
    : namedType
}

const getValidVariables = variables =>
  Object.keys(variables).filter(k => typeof variables[k] !== 'undefined')
...
export const buildGraphQLQuery = introspectionResults => (
  resource,
  aorFetchType,
  queryType,
  variables
